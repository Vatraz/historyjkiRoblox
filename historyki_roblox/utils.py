...
def remove_white_margins(input_path, output_path):
    ...
    non_transparent_pixels = [(x, y) for x in range(image.width) for y in range(image.height) if image_data.getpixel((x, y))[3] != 0]
    if non_transparent_pixels:
        ...
        padded_image.paste(cropped_image, ((max_side - cropped_image.width) // 2, (max_side - cropped_image.height) // 2))
